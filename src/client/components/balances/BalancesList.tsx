...
export function BalancesList({
  ...
  const errorMsg = [expenses, settlements]
    .map((e) => (e.isError ? e.error.message : undefined))
    .filter((e) => !!e)
    .at(0);

  if (isLoading || !expenses.data || !settlements.data)
    return <Loading name="balances" />;
        ...
        Array.from(owerMap).map(([owerId, amount]) => (
          ...
          >
            <div className="flex flex-col">
              <div>
                {memberIdToName.get(owerId) || 'ERROR'} owes{' '}
                {memberIdToName.get(owedId) || 'ERROR'}
              </div>
            ...
            <Button variant="outline" asChild>
              <Link
                to={addQueryParams(`/groups/${groupId}/settlements/new`, {
                  from: owerId,
                  to: owedId,
                  amount,
                })}
