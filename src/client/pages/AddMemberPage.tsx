...
function AddMemberPageCore({
  ...
  groupId: string;
  initialName?: string;
  mutation: UseMutationResult<any, Error, CreateMemberInput>;
}) {
  ...
  const onSubmit = (data: CreateMemberInput) => {
    mutation.mutate(data, { onSuccess: () => navigate(`/groups/${groupId}`) });
  };
    ...
    <Layout>
      <form onSubmit={handleSubmit(onSubmit)}>
        <Card className="w-full max-w-sm mx-auto">
          <CardHeader>
            <CardTitle>
              {initialName ? 'Update member' : 'Create virtual member'}
            </CardTitle>
...
export default function AddMemberPage() {
  ...
  return memberId ? (
    <UpdateMemberPage groupId={groupId} memberId={memberId} />
  ) : (
    <CreateMemberPage groupId={groupId} />
  );
}
