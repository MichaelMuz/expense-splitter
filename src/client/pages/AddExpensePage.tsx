...
function AddExpenseCore({
  ...
  groupId: string;
  initialData?: Expense;
  mutation: UseMutationResult<Expense, Error, CreateExpenseInput>;
}) {
...
function EditExpense({
  ...
  else if (expense.isError || !expense.data)
    return <p>Error loading expense {expense.error?.message || ''}</p>;

  return (
    <AddExpenseCore
      groupId={groupId}
      initialData={expense.data}
...
export default function AddExpensePage() {
  ...
  return expenseId ? (
    <EditExpense groupId={groupId} expenseId={expenseId} />
  ) : (
