...
function JoinGroupPageCore({
  inviteCode,
  unclaimedMembers,
}: {
  inviteCode: string;
  unclaimedMembers: GroupMember[];
}) {
  ...
  const onSubmit = (data: JoinInviteInput) => {
    joinGroup.mutate(
      { inviteCode, joinInput: data },
      { onSuccess: (result) => navigate(`/groups/${result.group.id}`) }
    );
  };
